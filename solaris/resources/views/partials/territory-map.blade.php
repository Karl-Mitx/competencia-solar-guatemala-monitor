...
<section class="card map-card {{ $expanded ?? false ? 'map-expanded' : '' }}">
    ...
    <div class="territory-toolbar"><label for="territory-select">Explorar territorio</label><select id="territory-select"><option value="">Guatemala · vista nacional</option>@foreach($territories as $territory)<option value="{{ $territory['code'] }}">{{ $territory['name'] }}</option>@endforeach</select><label for="map-health-filter">Lectura</label><select id="map-health-filter"><option value="all">Todas las granjas</option><option value="active">Solo activas</option><option value="inactive">Solo inactivas</option><option value="attention">Requieren atención (≤80%)</option><option value="healthy">Sobre 80%</option></select><button type="button" class="territory-reset" id="territory-reset">Ver todo el país ↗</button></div>
