<form class="filter-bar" method="GET" action="{{ url()->current() }}">
    ...
    <label><span class="sr-only">Departamento</span><select name="department_id"><option value="">Todos los departamentos</option>@foreach($departments as $department)<option value="{{ $department->id }}" @selected(($filters['department_id'] ?? '') == $department->id)>{{ $department->name }}</option>@endforeach</select></label>
    @if(isset($farms) && !($farms instanceof \Illuminate\Pagination\LengthAwarePaginator))<label class="farm-filter"><span class="sr-only">Granja</span><select name="solar_farm_id"><option value="">Todas las granjas</option>@foreach($farms as $filterFarm)<option value="{{ $filterFarm->id }}" @selected(($filters['solar_farm_id'] ?? '') == $filterFarm->id)>{{ $filterFarm->name }}</option>@endforeach</select></label>@endif
